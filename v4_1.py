...
def plot_chart(
    ...
    fig.text(
        0.28, 0.91,
        f'买入{buy_count}个(强{strong_count}/普{normal_count}/弱{weak_count}) | 卖出{sell_count}个 | 策略:量价MACD背离v4.1',
        fontsize=10, va='top'
    )
    fig.text(
        0.28, 0.88,
        '买点:底背离三重+零轴下+确认 | 卖点:顶背离三重+零轴上+确认',
        fontsize=9, va='top'
    )
...
def print_signals(signals: List[TradeSignal]) -> None:
    ...
    print()
    print('=' * 120)
    print('MACD分时策略v4.1 - 信号列表 (量价MACD三维背离完整版)')
    print('=' * 120)
    print()
    print(f"{'时间':<10} {'信号':<4} {'价格':<8} {'背离类型':<12} {'子类型':<8} {'零轴':<6} {'确认':<8} {'次数':<4} {'强度':<6} {'说明'}")
    print('-' * 120)
    ...
    for s in signals:
        ...
        print(
            ...
            f"{s.divergence_type:<12} {s.sub_type:<8} {s.zero_axis:<6} {confirm:<8} "
            f"{count:<4} {s.signal_level.value:<6} {s.reason}"
        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="MACD分时策略 v4.1 - 量价MACD三维背离完整版",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python v4_1.py --code 000559
  python v4_1.py --code 000559 --date 20260424
  python v4_1.py --random
        """
    )
...
def main() -> None:
    ...
    print("=" * 60)
    print("MACD分时策略v4.1 - 量价MACD三维背离完整版")
    ...
    print("检测信号(量价MACD三维背离完整版)...")
    signals = detect_signals(df, params)
    print_signals(signals)

    save_path = os.path.join(output_dir, f"MACD分时v4.1_{stock_code}_{detected_date}.png")
    print(f"\n绘图: {save_path}")
